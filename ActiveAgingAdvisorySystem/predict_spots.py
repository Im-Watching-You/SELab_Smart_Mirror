...
class AgingDiagnosis(threading.Thread):
    def __init__(self, q, img):
        ...
        path_aging_spot_model = './models/aging_factor/aging_spot_model.pkl'
        self.open_model(path_aging_spot_model)
    """
    ...
    def apply_sobel(self, img, ksize, thres=None):
        ...
        if thres is not None:
            ...
            return It   # With Threshold
        else:
            return Im   # Without Threshold

    # Calculate average value for Density; Max Spot Size on face, Total number of Spots on face. Only for Visualization.
    def calculate_spot_value(self, spots):
        ...
        # Max Spot Size on face
        for s in spots[0][5:10]:
            if s > spot_size:
                spot_size = s

        # Total number of Spots on face
    ...
    # Detect Aging Spots Using SimpleBlobDetector from OpenCV
    ...
    def extract_spots(self, window, threshold):
        ...
        # Image Preprocecing to Make Binary Image
        ...
        for k in keypoints:
            ...
            if int(k.size) > spots_size:
                spots_size = int(k.size) + 1    # Find Max Size of Spot

        # Calculate total sum of Aging Spots pixels in one Section
        ...
        spots_count = len(coordinates)      # Calculate Total Number of Spots
        density = float(Sa) / float(Pa)     # Calculate Spot Density
    ...
    # Method for Analyse Features and Prediction Age
    def analyse_factors(self):
        try:
            # Get 15 Features for Prediction
            features = self.extract_features(self.face)
            if features != None:
                y_pred = self.trained_model.predict(features)      # Predict Age
                sp_values = self.calculate_spot_value(features)    # Only for Visualization

                # Age: 1, Features: 15, Summarize Features: 1 ([Mean Density, Max Spot Size, Total Count Spots]) = 17
                self.result = {'Age': y_pred[0],
                               ...
                               "Count_of_Spots_Corner_Left_Eye": int(features[0][10]*100),
                               ...
                               "Count_of_Spots_Cheek_Right": int(features[0][14]*100),  # Convert '0,03' to '3' spots
    ...
    # Run Threading
    def run(self):
        self.queue.put(self.analyse_factors())
