...
class ProgressAnalyzer:
    ...
    def _retrieve_user_variation_tmp_2(self, user_id, assess_type, duration):
        """
         To return two kind of rows
            (1) rows with start_date.
            (2) rows with end_date.
        ...
        if duration is 'week':
            ...
            # print(start_date)
        elif duration is 'month':
            start_date = datetime.today().date() - relativedelta(months=1)
        elif duration is 'quarter':
            start_date = datetime.today().date() - relativedelta(months=3)
        elif duration is 'year':
            start_date = datetime.today().date() - relativedelta(months=12)
        elif isinstance(duration, int):
            start_date = datetime.today().date() - timedelta(days=duration)
        else:
            print("ERROR: WC.progression analyzer.retrieve_user_variation : invalid duration.")
            return [], []

        end_date = datetime.now().date()

        dict_duration = {'start_date': start_date, 'end_date': end_date}
...
class AgingProgressAnalyzer(ProgressAnalyzer):
    ...
    def get_seasonal_variation_of_age_pre(self, user_id, duration):
        """
        :param user_id: Integer
        ...
        """
        start_date_rows, end_date_rows = self._retrieve_user_variation_tmp_2(user_id, self.assess_type, duration)
    ...
    def get_trend_of_age(self, user_id, user_name, dict_duration, interval='day', dict_fact=None):
        """
...
class EmotionProgressAnalyzer(ProgressAnalyzer):
    ...
    def get_top_emotions(self, user_id, duration=None):
        """
