...
def compare_prediction(val_true, val_pred):
    ...
    result = {"rmse":rmse, "mean": mean, "std":std, "var": var, "age_accuracy": round(1-(mean/100),2), "accuracy": acc}
    return result
...
print("# of whole test set size: ", len(test))
test = test.loc[(20 <= data['Age']) & (data['Age'] <= 50)]
print("# of part test set size: ", len(test))
...
clf = SVR(gamma='scale', C=500, epsilon=0.2, kernel='poly')
...
print("===========================================================")
start_time = time.time()
neigh = KNeighborsRegressor(n_neighbors=161)
