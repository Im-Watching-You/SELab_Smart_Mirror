...
class FactorDetector:
    ...
    def th_detect_emotion_distance_factor(self, frame):
        def th_compute_distance(frame):
            ...
            try:
                self.result_display, self.emotion_label, self.value_predicted = self.detect_emotion_computation(rects, gray)
            except (ValueError, AttributeError):
                error = 'Distance could not be computed'
                print(error)
                return error
        try:
            t = threading.Thread(target=th_compute_distance, args=(frame,))
            t.start()
        except:
    ...
    def detect_emotion_computation(self, rects, gray):
        ...
        try:
            ...
            error = 'Factors could not be computed.'
            print(error)
            return error
